		...
		<div
			...
			<form method="post" action="admin/record/update.htm"
				...
				<div class="form-group my-3 text-center" data-plugin="formMaterial">
					<h3>
						<c:choose>
							<c:when test="${empty message}"><s:message code="record.update"/></c:when>
							<c:otherwise>
								${message}
				...
				<div class="row form-group">
					...
					<input type="hidden"  name="staff.id" value="${record.staff.id}">
					<label class="form-control-label">Staff Name</label>
					<input type="text" class="form-control" value="${record.staff.name}" readonly>
				</div>
					...
					<div class="form-control bg-white">
						<div class="form-check form-check-inline">
							<input class="form-check-input" type="radio" name="type"
								id="inlineRadio1" value="true" ${record.type?"checked":""}> <label
								class="form-check-label" for="inlineRadio1"><s:message code="record.type.prise"/></label>
						</div>
						<div class="form-check form-check-inline">
							<input class="form-check-input" type="radio" name="type"
								id="inlineRadio2" value="false" ${record.type?"":"checked"}> <label
				...
				<div class="row form-group my-3">
					<label class="form-control-label" for="inputDepart"><s:message code="record.date"/></label> <input
						...
						name="date" placeholder="Depart Name" value="${record.date}" required>
					<div class="invalid-feedback">Invalid Date</div>
				</div>

				<div class="row form-group my-3">
					<label for="inputReason"><s:message code="record.reason"/></label>
					<textarea class="form-control" id="inputReason" name=reason rows="3" required>${record.reason}</textarea>
				...
				<div class="form-control-plaintext my-3">
					<button class="btn btn-primary mb-3 px-3 mx-2" type="submit">
						<i class="fas fa-plus"></i> <s:message code="record.update"/>
					</button>
